...
export class Lanyard extends EventEmitter implements Lanyard {
    ...
    public fetch(): Promise<LanyardPresence> {
        return fetch(`https://api.lanyard.rest/v1/users/${this.user_id}`,).then(res => res.json()).then(data => data.data);
    }
...
export const lanyard = typeof window !== "undefined" ? new Lanyard('678203623708033024') : null
